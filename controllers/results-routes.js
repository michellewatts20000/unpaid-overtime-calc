...
router.get('/:id', async (req, res) => {
  const userSumIndustry = await Entry.findAll({
    attributes: [
      'industry',
      [
        sequelize.fn('sum', sequelize.col('unpaid_hours')),
        'total_unpaid_hours',
      ],
      [
        sequelize.fn('sum', sequelize.col('unpaid_salary')),
        'total_unpaid_salary',
      ],
  ...
  const users4 = userSumAll$.map((user) =>
    user.get({
      plain: true,
    })
  );
  ...
  const average = (totals.total_unpaid_salary / totals.total_submissions).toFixed(2);
